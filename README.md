...
level -- so there is no need to calculate them once the level index is determined. See the table below for Tomb Raider I offsets. Note, they are relative offsets.
...
to two possible locations of the 0xFF array. The second location is 0xA bytes apart from the first. See the code below.
